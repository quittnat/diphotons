...
for cut in "${cuts[@]}" ;do
	echo $cut
	if [[ $cut -eq 0 ]]; then
		output=full_analysis_moriond16v1_sync_v5_extra_vars_0_500
		cut='(mass < 500.)'
	elif [[ $cut -eq 1 ]]; then
		output=full_analysis_moriond16v1_sync_v5_extra_vars_500_730
		cut='(mass > 500 && mass < 730.)'
	elif [[ $cut -eq 2 ]]; then
		output=full_analysis_moriond16v1_sync_v5_extra_vars_730_790
		cut='(mass > 730 && mass < 790)'
	elif [[ $cut -eq 3 ]]; then
		output=full_analysis_moriond16v1_sync_v5_extra_vars_790_Inf
		cut='(mass > 790.)'
	fi
	mkdir $output
