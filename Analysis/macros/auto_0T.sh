...
for cut in "${cuts[@]}" ;do
	echo $cut
	if [[ $cut -eq 0 ]]; then
		output=full_analysis_moriond16v1_0T_sync_v6_230_or_300_500
		cut='((mass < 500.)'
  ...
  ## merge in Data
	...
	./basic_plots.py --load basic_plots_0T.json -O /afs/cern.ch/user/m/mquittna/www/diphoton/$output/ --input-dir $output  --fudge $fudge --lumi 0.6 
done
