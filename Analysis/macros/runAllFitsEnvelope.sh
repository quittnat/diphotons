...
cp $bkgfile $folder/.
./combine_maker.sh $file2 --luminosity 2.56 --lumi 2.56 --generate-signal --fit-name cic2 --generate-datacard --rescale-signal-to 1e-3 --parametric-signal srm://t3se01.psi.ch/pnfs/psi.ch/cms/trivcat/store/user/mquittna/nominalWSwithSmear_k001_m500to998.root --parametric-signal-acceptance acceptance_pu.json --background-root-file $bkgfile --use-envelope
